...
This formulation is called mean-parametrization, as the denoising network is attempting to predict the mean of $x_0$ given the noisy input $x_t$. Intuitively, \eqref{eq:reverse process x_0 unknown} can be thought of as if $x_t$ is a mask token, with probability $\frac{\alpha_s - \alpha_t}{1 - \alpha_t}$, we sample a new token from $\mu_\theta(x_t)$, otherwise we keep the token from $x_t$. This allows us to sample new sequences from the network by iteratively converting mask tokens into other tokens via sampling from \eqref{eq:reverse process x_0 unknown}. We describe the sampling procedure in more detail in section \ref{sec:sampling}.
...
\begin{align*}
    1 &= \mathbb{E}_{y\sim \pi_\theta}\left[\frac{\pi_\text{ref}(y)}{\pi_\theta(y)}\right] \\
    &= \mathbb{E}_{y\sim \pi_\theta}\left[e^{-\log\frac{\pi_\theta(y)}{\pi_\text{ref}(y)}}\right] \\
    &= \mathbb{E}_{y\sim \pi_\theta}\left[\sum_{n=0}^{\infty}\frac{(-1)^n \left( \log\frac{\pi_\theta(y)}{\pi_\text{ref}(y)} \right)^n}{n!}\right] \\
    &\approx \mathbb{E}_{y\sim \pi_\theta}\left[1 - \log\frac{\pi_\theta(y)}{\pi_\text{ref}(y)} + 0.5\cdot\left( \log\frac{\pi_\theta(y)}{\pi_\text{ref}(y)} \right)^2\right] \\
    &= 1 - \mathbb{E}_{y\sim \pi_\theta}\left[\log\frac{\pi_\theta(y)}{\pi_\text{ref}(y)}\right] + 0.5\cdot\mathbb{E}_{y\sim \pi_\theta}\left[\left( \log\frac{\pi_\theta(y)}{\pi_\text{ref}(y)} \right)^2\right].
\end{align*}
...
One of the main functions of chess engines is to evaluate a given position. This is usually done in the centipawn metric, which indicates the amount of material or tactical advantage a player has in terms of the value of a pawn. For instance, if white is down a pawn, which typically results in a baseline evaluation of $-100$ centipawns, but has a forced sequence threatening checkmate compelling black to give up a queen to avoid losing, the engine will look past the immediate material deficit and evaluate the position at roughly $+800$ centipawns. The probability of winning can be defined in many ways, but in this thesis, we use the definition of Lichess \cite{Lichessaccuracymetric}, which applies the logistic function to the centipawn evaluation as follows:

% \begin{equation}
%     \mathbb{P}(\text{win}) = 50 + 50\cdot\left(\frac{2}{1 + \exp(-0.00368208\cdot\text{centipawns})} - 1\right).
%     \label{eq:prob_win}
% \end{equation}
\begin{equation}
    \mathbb{P}[\text{win} | a, s] = \frac{1}{1 + \exp(-0.00368208\cdot\text{eval}(a | s))},
    \label{eq:prob_win}
\end{equation}
where $s$ is the board state and $a$ is the move to be made. $\text{eval}(a | s)$ is the centipawn evaluation of the position after the move $a$ has been made. The evaluation is relative to the side to move meaning that if the side who makes the move $a$ is winning, the evaluation is positive. Otherwise, the evaluation is negative.
