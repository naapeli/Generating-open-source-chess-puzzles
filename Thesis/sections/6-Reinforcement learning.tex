...
A better estimate for the KL divergence can be constructed by noting that $\mathbb{E}_{y\sim \pi_\theta}[\frac{\pi_\text{ref}(y)}{\pi_\theta(y)} - 1] = 0$ \cite{schulman2020klblog}. Schulman uses this to produce another unbiased estimate, that has lower variance compared to $k_1$. The estimate is as follows:
...
In this thesis, we employ $k_2$ to estimate the KL divergence between the new and the reference policy. As we use diffusion models and the ELBO as a biased surrogate for the log probability of the generated tokens, the KL estimate is biased in all cases. Hence, we prioritize low variance and stable gradients.
