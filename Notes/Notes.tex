...
% def compute_loss_quadrature_vectorized(model, fens, themes, ratings, K=3):
%     device = fens.device
%     B = fens.size(0)
%     t_nodes, t_weights = numpy.polynomial.legendre.leggauss(K)
%     t = t_nodes[:, None].expand(K, B)
%     alpha_t = config.masking_schedule(t).unsqueeze(-1)

%     fens_rep = fens.unsqueeze(0).expand(K, *fens.shape)
%     themes_rep = themes.unsqueeze(0).expand(K, *themes.shape)
%     ratings_rep = ratings.unsqueeze(0).expand(K, *ratings.shape)

%     random_mask = torch.rand_like(fens_rep, dtype=torch.float) < alpha_t
%     masked_fens = torch.where(random_mask, fens_rep, config.mask_token)
%     masked_fens = masked_fens.flatten(0, 1)
%     themes_rep = themes_rep.flatten(0, 1)
%     ratings_rep = ratings_rep.flatten(0, 1)
%     t_flat = t.flatten(0, 1)

%     with torch.autocast(device_type=device_type, dtype=torch.bfloat16):
%         logits = model(masked_fens, themes_rep, ratings_rep)

%     unwrapped_model = model.module if distributed else model
%     loss = unwrapped_model.elbo_loss(
%         t_flat, logits, fens_rep.flatten(0, 1), masked_fens
%     )

%     loss = loss.view(K, B)
%     return torch.sum(t_weights[:, None] * loss, dim=0)
