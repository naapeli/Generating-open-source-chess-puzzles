...
\begin{figure}
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includesvg[width=\textwidth]{figures/rating_model_errors63000.svg}
    \end{subfigure}
    \caption{Errors up to 20 000 steps (left) and 63 000 (right). In the orange histogram, the themes used to help the regression are shuffled to make the model's dependence on the themes clearer.}
\end{figure}

The average absolute error for the best rating model on the test set is 202 elo points and the median error is 257 elo points. Hence, we can probably confidently use it in the reward function.

\section{3.3}

The puzzles generated with the new supervised model (step 760,000 from 1,000,000) yielded better results over 10 000 generated positions with the themes taken from the Lichess puzzle dataset randomly.

\begin{table}[h]
    \centering
    \begin{tabular}{|c|c|c|c|c|}
        \hline
        & Lichess puzzles & Masked Diffusion & Last & Paper \\
        \hline
        Legal & 100\% & 97.3\% & 96.8\% & 99.72\%\\
        \hline
        Unique & 81.7\% (95.25\%) & 17.6\% & 9.67\% & 30.89\%\\
        \hline
        Counter-intuitive & 4.3\% (2.25\%) & 2.7\% & 23.7\% & 1.11\%\\
        \hline
        Puzzle & 4.1\% (2.14\%) & 0.47\% & 0.1\% & 0.34\%\\
        \hline
    \end{tabular}
\end{table}

These positions have more overlap with the Lichess dataset than previous 0.12\%, but the board Levenshtein distance is still higher than with the paper at 13.19 (average minimum distance between the generated positions and the Lichess dataset, which was 8.528 in the paper).

The distance between the Lichess puzzle dataset to itself is 13.12 (11.914 in the paper). The implementations may differ slightly, I only average over 100 positions.

\section{16.3}

In RL, the uniqueness and counter intuitiveness improve when using batch size 16, group size 8, eps 0.3, beta $5\cdot 10^{-4}$, lr $3\cdot 10^{-5}$ and weight decay $10^{-5}$. The reward was -2 for illegal positions, 0.5 for unique but not counter-intuitive positions, 2 for unique and counter-intuitiveness positions and 0 otherwise.

The distribution of themes affects the performance of the model a lot. Before RL, the Lichess puzzle dataset theme distribution produces much better results, however, after RL with my own theme distribution, the performance on my own distribution gets much better, but on the Lichess distribution does not increase. After RL, the uniqueness is around 8\%, while the counter-intuitiveness (filtered for unique positions) is around 10\% ($p(\text{counter-intuitive} | \text{unique}) \approx 10\%$). In total, the proportion of puzzles, which pass both uniqueness and counter-intuitiveness is around 0.8\% with potential for further improvement. The proportion of exact overlap with the Lichess puzzles dataset is around 0.05\% or 5 positions from 9216. The board Levenshtein distance to the Lichess dataset is around 13.79, which is slightly higher than before RL training despite the reward having no diversity filtering. After looking at the actual generated puzzles, around 80\% of the ones that meet both uniqueness and counter-intuitiveness criteria are double check puzzles (with the same principal variation).


\section{17.3}

When we give -2 for illegal, 0.5 for unique, 2 for unique and counter-intuitive and 0 reward otherwise, the counter intuitiveness and uniqueness both increase, but the diversity decreases (80\% of the good puzzles generated by the model have the double check theme with the same solution or principal variation). After modifying the reward such that for positive reward, the generation must have a different PV first move than all others in the batch, the uniqueness still increases, but the counter-intuitiveness doesn't really increase meaningfully. Instead, the PV distance between the puzzles in the batch increases steadily.

\section{18.3}

The model generated a great position while training:

\begin{figure}[h]
    \centering
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/HighlightPos1803.png}
        \caption{\footnotesize 6rk/p2R3p/2p4r/6p1/8/2N1KQq1/\allowbreak PP6/\allowbreak 3R4 b - - 1 27 \protect\footnotemark}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/HighlightPos1803v2.png}
        \caption{\footnotesize r2kn3/pp3R2/2pbp2q/8/3PN3/4PQ2/\allowbreak PP3P1P/\allowbreak 7K w - - 1 22 \protect\footnotemark}
    \end{subfigure}    
\end{figure}


\addtocounter{footnote}{-1}
\footnotetext{Solution: 1.Re6+ Ne4 2.Rxe4+ Kxe4 3.Re8+ Kd3 4.Qxf3+}
\stepcounter{footnote}
\footnotetext{Solution: 1.Nxd6 Nxd6 2.Rf8+ Kc7 3.Rxa8}


\section{20.3}

I realized that the explanation of division by sequence length in \cite{ou2025principledrldiffusionllms} is bad, as it only shows that the division by the sequence length happens for the RL loss and not the kl divergence. Hence, in the previous implementations I have had, the KL coefficient $\beta$ is almost 10000 times larger than what was chosen in \cite{feng2025generatingcreativechesspuzzles} ($1 / 76^2\approx1.73\cdot 10^{-4}$). Authors of \cite{rojas2025improvingreasoningdiffusionlanguage} do not divide by the sequence length, but otherwise the algorithm is very similar to ESPO. Hence, we remove the division by the sequence length (as it is constant as well in our use case). This way the algorithm matches more closely the one in \cite{feng2025generatingcreativechesspuzzles}.


\section{24.3}

I realized that since \cite{feng2025generatingcreativechesspuzzles} works with token-level KL-divergences, their KL-divergence is a lot smaller than what I have. Their is a sum off small divergences, whereas mine is one larger divergence ($x_1 ^ 2  + \dots + x_n ^ 2 \leq (x_1 + \dots + x_n) ^ 2, x_i \geq 0$).

\section{10.4}

I think I will attempt to train the model to play as well as possible with RL (only mask the tokens of the best move) and see if the ability to generate puzzles changes.

\section{13.4}

After the long tuning run with legal * (10 * passDiversity + uniqueness + 0.5 * counterIntuitiveness) reward, I notice that the model starts to generate only mates-in-one, if the hyperparameters are chosen in the best way, as they always have a unique solution. I think I will increase the weight of the counterIntuitiveness, as it is only given if the position is unique and has a counter intuitive solution. I might try legal * passDiversity * (uniqueness + 10 * counterIntuitiveness) or something.

\section{14.4}

The model that also predicts moves has 129 average cp loss for positions with no checkmate. For checkmates, it finds the correct mate 52\% of the time. In all positions combined, the best move is found 83\% of the time. The model generates an illegal move around 1.75\% of the time (depends also on the amount of steps, which in this case was quite low). For positions that are not puzzles, we cannot give the model information about the theme or the difficulty of the position. Hence, the cp loss for such positions is around 322 (a knights worth) and the move accuracy is around 15\%. The model finds a correct checkmate for around 28\% of the time and generates a legal move about 85\% of the time. Clearly the theme information helps the model quite a lot.
