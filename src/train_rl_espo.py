...
from rl.espo import compute_elbo, compute_elbo_basic, kl_estimate, entropy, generate_random_themes, theme_reward
...
def get_reward(x_t, entropy, config, step, themes_tokens=None, ratings=None):
    ...
    if config.use_context:
        ...
        themes_match = torch.ones(batch_size, dtype=bool)
    ...
    for i, (fen, move) in enumerate(generations):
        ...
        pv = pvs[i]
        intra_batch_fen_dist[i], intra_batch_pv_dist[i], _, _ = intra_batch_distances(fen, pv, batch_fens, batch_pvs, i)
    ...
    pass_diversity_filtering = good_intra_fen & good_inter_fen & good_intra_pv & good_inter_pv & (entropy > 5.2)
...
def train_espo(model, reference_model, optimizer, scheduler, config, device, args, step, master_process, distributed, world_size, local_batch_size, local_n_artificial):
    model.eval()
    ...
    with torch.autocast(device_type=device.type, dtype=torch.bfloat16):
        ...
        step_fens = sample_model.sample(themes_one_hot, scaled_ratings, steps=args.steps, batch_size=local_batch_size * args.group_size, temperature=args.temperature, generate_move_last=False, compute_kl=False, compute_entropy=False)
    
    if local_n_artificial > 0:
        sampled_fens, sampled_pvs, sampled_themes, sampled_ratings = buffer.sample(local_n_artificial)
        x_0_art_list = []
    ...
    with torch.no_grad():
        ...
        reference_elbo = compute_elbo(reference_model, step_fens, themes=themes_one_hot, ratings=scaled_ratings, mask=mask, return_mask=False)

    sequence_length = step_fens.shape[1]
    old_entropy = entropy(old_elbo, sequence_length)
    scaled_entropy = old_entropy + np.log(args.steps)

    local_exact_kl = kl_estimate(old_elbo, reference_elbo, kind="k2").mean()
    local_exact_entropy = scaled_entropy.mean()
    if distributed:
    ...
    if distributed:
        ...
        gather_entropies = [torch.zeros_like(old_entropy) for _ in range(world_size)] if master_process else None
        
        gather(step_fens, gather_list=gather_fens, dst=0)
        gather(old_entropy, gather_list=gather_entropies, dst=0)
        ...
        if master_process:
            ...
            global_scaled_entropies = global_entropies + np.log(args.steps)
            ...
            global_rewards = get_reward(global_fens, global_scaled_entropies.cpu(), config, step, themes_tokens=global_themes, ratings=global_ratings)
        ...
        rewards_gen = get_reward(step_fens, scaled_entropy.cpu(), config, step, themes_tokens=themes_one_hot, ratings=ratings)
    ...
    for substep in range(args.n_gradient_updates_per_generation):
        ...
        kl = kl_estimate(elbo_gen, reference_elbo, kind="k2").mean() / sequence_length
        entropy_val = (entropy(elbo_gen, sequence_length).mean() + np.log(args.steps)) / sequence_length
        ...
        advantages_grouped = (r_gen_grouped - mean_other_rewards) / (r_gen_grouped.std(dim=1, keepdim=True) + 1e-5)
        advantages = advantages_grouped.reshape(-1).to(device)

        log_ratio = torch.clamp((elbo_gen - old_elbo) / sequence_length, min=-10.0, max=10.0)
        ...
        loss = loss_ppo + args.sup_loss_coef * loss_sft + args.kl_coef * kl - args.entropy_coef * entropy_val
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n_artificial", "--n_arteficial", type=int, default=4, dest="n_artificial")
    parser.add_argument("--n_gradient_updates_per_generation", "--ppo_epochs", type=int, default=1, dest="n_gradient_updates_per_generation")
    parser.add_argument("--steps", type=int, default=256)
    parser.add_argument("--temperature", type=float, default=1.0)
    parser.add_argument("--kl_coef", "--beta", type=float, default=0.03, dest="kl_coef")
    ...
    parser.add_argument("--sup_loss_coef", "--gamma", type=float, default=1.0, dest="sup_loss_coef")
    ...
    local_n_artificial = args.n_artificial // world_size
    ...
    try:
        for step in range(start_step, args.n_generations):
            train_espo(model, reference_model, optimizer, scheduler, config, device, args, step, master_process, distributed, world_size, local_batch_size, local_n_artificial)
